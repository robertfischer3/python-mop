...
class AzureSubscriptions:
    ...
    def subscription_list(self):
        ...
        subscription_list = [[subscription_item.display_name, subscription_item.subscription_id] for subscription_item
                             in
    ...
    def get_subscription(self, creds, subscription_id):
        ...
        '''
        subscription_client = SubscriptionClient(self.credentials, base_url=None)
        sub_info = subscription_client.subscriptions.get(subscription_id=subscription_id)
        return sub_info
    ...
    def list_management_grp_subcriptions(self, management_grp):
        ...
        subscriptions_limited = [[subscriptions.name,
    ...
    def list_management_grp_subscriptions_list(self, management_grp, subscription_id):
        ...
        for subscription in mngrp_subscriptions:
            if '/providers/Microsoft.Management/managementGroups' not in subscription.type:

                policy_client = PolicyClient(credentials=self.credentials, subscription_id=subscription_id, base_url=None)
                policies = policy_client.policy_definitions.list_by_management_group(management_grp)

                scope = '/subscriptions/{subscriptionId}'.format(subscriptionId=subscription.name)

                for policy in policies:
                    if not policy.metadata.get('category') is None and\
                            policy.metadata.get("category") in "Nestle Security":

                        assignment = PolicyAssignment(display_name=policy.display_name,
                        ...
                        policy_assignment_name = "{}-assignment".format(assignment.display_name)
                        try:
                            ...
                            if result:
                                print("Subscription deployed: {}".format(subscription.name, policy_assignment_name, policy.display_name))
                        except ErrorResponseException:
                            print('Subcription failed: {}'.format(subscription.name, policy_assignment_name, policy.display_name))
